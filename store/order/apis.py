...
def add_order():  # Real Project this section session or temp memory.
    add_order_schema = OrderSchema()
    valid_data = add_order_schema.load(request.get_json())

    product_ids = [item.get("product_id") for item in valid_data.get("items")]
    products = (
        db.session.query(
            Product.id,
            Product.price,
    ...
    order = add_order_schema.create_order(order_data)
    ...
    item_objects = [
        ...
        for item in valid_data.get("items")
...
def update_pending_order(order_id):
    ...
    order_schema = OrderSchema()
    valid_data_order = order_schema.load(request.get_json())

    product_ids = [item.get("product_id") for item in valid_data_order.get("items")]
    products = (
    ...
    item_objects = [
        ...
        for item in valid_data_order.get("items")
        if (product := map_products.get(item.get("product_id")))
    ]
    db.session.bulk_save_objects(item_objects)
